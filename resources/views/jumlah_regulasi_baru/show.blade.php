...
<div class="bg-white p-6 rounded-lg shadow-md">
    ...
    <div class="space-y-4 text-sm">
        <div>
            ...
            <span class="text-gray-800">{{ $jumlahRegulasiBaru->tahun }}</span>
        </div>
        <div>
            <strong class="text-gray-600 w-48 inline-block">Bulan:</strong> 
            <span class="text-gray-800">{{ \Carbon\Carbon::create()->month($jumlahRegulasiBaru->bulan)->isoFormat('MMMM') }}</span>
        </div>
        
        {{-- Menampilkan teks dari accessor di Model --}}
        <div class="border-t pt-4 mt-4">
            <strong class="text-gray-600 w-full block mb-1">Substansi:</strong> 
            <p class="text-gray-800">{{ $jumlahRegulasiBaru->substansi_text }}</p>
        </div>
        <div class="border-t pt-4 mt-4">
            <strong class="text-gray-600 w-full block mb-1">Jenis Regulasi:</strong> 
            <p class="text-gray-800">{{ $jumlahRegulasiBaru->jenis_regulasi_text }}</p>
        </div>
        
        <div>
            <strong class="text-gray-600 w-48 inline-block">Jumlah Regulasi:</strong> 
            <span class="text-gray-800">{{ number_format($jumlahRegulasiBaru->jumlah_regulasi) }}</span>
        </div>
        
        <div class="border-t pt-4 mt-4"></div>
        <div>
            <strong class="text-gray-600 w-48 inline-block">Dibuat pada:</strong> 
            <span class="text-gray-800">{{ $jumlahRegulasiBaru->created_at->isoFormat('D MMMM YYYY, HH:mm') }}</span>
        </div>
        <div>
            <strong class="text-gray-600 w-48 inline-block">Diperbarui pada:</strong> 
            <span class="text-gray-800">{{ $jumlahRegulasiBaru->updated_at->isoFormat('D MMMM YYYY, HH:mm') }}</span>
        </div>
    </div>
