    ...
    <form method="GET" action="{{ route('phi.perselisihan-ditindaklanjuti.index') }}" class="w-full">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-3 items-end">
            ...
            <div class="flex items-center space-x-2 pt-5">
                ...
                <a href="{{ route('phi.perselisihan-ditindaklanjuti.index') }}" class="w-full sm:w-auto px-4 py-1.5 bg-gray-200 text-gray-700 rounded-button hover:bg-gray-300 text-sm font-medium">
                    Reset
                </a>
            </div>
        </div>
...
<div class="bg-white p-4 sm:p-6 rounded-lg shadow-md">
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div class="w-full sm:w-auto sm:ml-auto flex flex-col sm:flex-row items-start sm:items-center gap-2">
            ...
            <a href="{{ route('phi.perselisihan-ditindaklanjuti.create') }}" class="w-full sm:w-auto flex items-center justify-center px-4 py-2 bg-primary text-white rounded-button hover:bg-primary/90 text-sm font-medium whitespace-nowrap mt-2 sm:mt-0">
                <i class="ri-add-line mr-1"></i> Tambah Data Perselisihan
            </a>
        </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200 border border-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($perselisihanDitindaklanjutis as $index => $item)
                    <tr>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{{ $perselisihanDitindaklanjutis->firstItem() + $index }}</td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700 text-right">{{ number_format($item->jumlah_ditindaklanjuti) }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-center">
                            <div class="flex items-center justify-center space-x-2">
                                ...
                                {{-- <a href="{{ route('phi.perselisihan-ditindaklanjuti.show', $item->id) }}" class="text-gray-500 hover:text-gray-700" title="Lihat Detail">
                    ...
                    <tr>
                        <td colspan="10" class="px-4 py-10 text-center text-sm text-gray-500">
                            <div class="flex flex-col items-center">
                                ...
                                Tidak ada data perselisihan ditemukan.
                            </div>
                        </td>
                    </tr>
                @endforelse
