...
<div class="bg-white p-6 rounded-lg shadow-md">
     <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-semibold text-gray-800">Formulir Edit Data Lowongan Pasker</h2>
        <a href="{{ route($routeNamePrefix . 'index') }}" class="text-sm text-primary hover:text-primary/80 flex items-center">
            <i class="ri-arrow-left-line mr-1"></i> Kembali ke Daftar
        </a>
    </div>
    <form action="{{ route($routeNamePrefix . 'update', $jumlahLowonganPasker->id) }}" method="POST">
        @method('PUT')
