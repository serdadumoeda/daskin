    ...
    <form method="GET" action="{{ route('sekretariat-jenderal.progress-mou.index') }}" class="w-full">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 items-end">
            ...
            <div class="flex items-center space-x-2 pt-5">
                ...
                <a href="{{ route('sekretariat-jenderal.progress-mou.index') }}" class="btn-secondary-outline">
                    Clear Filter
                </a>
            </div>
        </div>
    ...
    @if (Auth::user()->role === 'superadmin' || Auth::user()->role === 'sekjen')
        <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
            ...
            <div class="w-full sm:w-auto sm:ml-auto flex flex-col sm:flex-row items-start sm:items-center gap-2">
                ...
                <a href="{{ route('sekretariat-jenderal.progress-mou.create') }}" class="btn-primary">
        ...
        <table class="data-table">
            <thead>
                ...
                <tr>
                    ...
                    <th scope="col">{!! sortableLinkMou('tahun', 'Tahun', $sortBy, $sortDirection, $requestFilters) !!}</th>
                    <th scope="col">{!! sortableLinkMou('bulan', 'Bulan', $sortBy, $sortDirection, $requestFilters) !!}</th>
                    <th scope="col">{!! sortableLinkMou('judul_mou', 'Judul MoU', $sortBy, $sortDirection, $requestFilters) !!}</th>
                    <th scope="col">{!! sortableLinkMou('tanggal_mulai_perjanjian', 'Tgl Mulai', $sortBy, $sortDirection, $requestFilters) !!}</th>
                    <th scope="col">{!! sortableLinkMou('tanggal_selesai_perjanjian', 'Tgl Selesai', $sortBy, $sortDirection, $requestFilters) !!}</th>
                    <th scope="col">{!! sortableLinkMou('pihak_terlibat', 'Pihak Terlibat', $sortBy, $sortDirection, $requestFilters) !!}</th>
            ...
            <tbody>
                ...
                @forelse ($progressMous as $index => $item)
                    <tr>
                        ...
                        <td class="max-w-xs truncate" title="{{ $item->judul_mou }}">{{ Str::limit($item->judul_mou, 50) }}</td>
                        <td>{{ $item->tanggal_mulai_perjanjian ? \Carbon\Carbon::parse($item->tanggal_mulai_perjanjian)->isoFormat('D MMM YYYY') : '-' }}</td>
                        <td>{{ $item->tanggal_selesai_perjanjian ? \Carbon\Carbon::parse($item->tanggal_selesai_perjanjian)->isoFormat('D MMM YYYY') : '-' }}</td>
                        <td class="max-w-xs truncate" title="{{ $item->pihak_terlibat }}">{{ Str::limit($item->pihak_terlibat, 50) }}</td>
                    ...
                    <tr>
                        <td colspan="8" class="text-center py-10">
                            <div class="flex flex-col items-center text-gray-500">
                                ...
                                <span>Tidak ada data MoU ditemukan.</span>
                            </div>
                        </td>
                    </tr>
                @endforelse
